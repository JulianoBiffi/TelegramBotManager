    ...
    ILogger<CreditCardClosingDateManagementHandler> _logger) : IRequestHandler<CreditCardClosingDateManagementCommand, Unit>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Validar bankName obrigatório
        if (string.IsNullOrEmpty(bankName))
        {
            await SendErrorMessage(
                "⚠️ *Erro: Parâmetro obrigatório ausente*\n\n" +
        ...
        {
            // Criar novo registro - ambos parâmetros são obrigatórios
            if (!closingDateValue.HasValue || !bestDayToBuyValue.HasValue)
            {
                await SendErrorMessage(
                    "⚠️ *Erro: Parâmetros obrigatórios ausentes*\n\n" +
        ...
        CancellationToken cancellationToken)
    {
        ...
        message.AppendLine("━━━━━━━━━━━━━━━━━━━━━━━");
        message.AppendLine($"🏛️ *Banco:* {entity.BankName}");
        message.AppendLine($"📅 *Dia de Fechamento:* {entity.ClosingDate}");
        message.AppendLine($"🛍️ *Melhor Dia para Compra:* {entity.BestDayToBuy}");
